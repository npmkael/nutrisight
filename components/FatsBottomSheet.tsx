...
interface FatsBottomSheetProps {
...
function FatsBottomSheet({
  bottomSheetRef,
  currentValue,
  onSave,
  onClose,
}: FatsBottomSheetProps) {
  ...
  const handleIncrement = useCallback(() => {
    ...
    const newValue = Math.min(currentNum + 5, 200);
    setValue(newValue.toString());
  }, [value]);

  const handleDecrement = useCallback(() => {
    const currentNum = parseFloat(value) || 0;
    const newValue = Math.max(currentNum - 5, 0);
    setValue(newValue.toString());
  }, [value]);
...
export default memo(FatsBottomSheet);
